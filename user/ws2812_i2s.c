...
#define WS_I2S_BCK 23  //Seems to work as low as 18, but is shakey at 1.
#define WS_I2S_DIV 3
#elif defined( WS2812_FOUR_SAMPLE ) || defined(SK6812)
#define WS_I2S_BCK 17  //Seems to work as low as 13, shoddy at 12.
#define WS_I2S_DIV 3
#else
...
static unsigned int i2sZeroes[256];
...
void ICACHE_FLASH_ATTR ws2812_disable()
{
	//Disable DMA
...
void ICACHE_FLASH_ATTR ws2812_init()
{
	...
	i2sBufDescZeroes.datalen = 256;
	i2sBufDescZeroes.blocksize = 256;
	...
	for( x = 0; x < 256; x++ )
	{
		i2sZeroes[x] = 0xffffffff;   //XXX TRICKY: Need to set to 0xff, otherwise, we we'll corrupt the state when entering/exititng sleep.
	}
...
#ifdef WS2812_THREE_SAMPLE
	uint8_t * bufferpl = (uint8_t*)&i2sBlock[0]; //Start a good bit in, to avoid the FFs and create a good "MARK"
